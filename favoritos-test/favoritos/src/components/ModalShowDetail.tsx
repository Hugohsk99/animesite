...
const ModalShowDetail: React.FC<ModalShowDetailProps> = ({
  ...
  const handleDelete = async (anime: Anime) => { 
    const response = await fetch(`${url}/favoritos/users/animes/${anime.mal_id}/delete`, {
      method: 'POST',
      headers: {
  ...
  const handleSubmit = async () => {
    const response = await fetch(`${url}/favoritos/users/animes/${anime?.mal_id}/edit`, {
      method: 'PATCH',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${localStorage.getItem('token')}`,
            ...
            {showDataEdit && (
              <div id="data-edit">
                <Box sx={{ display: 'flex', flexDirection: 'column', marginTop: '1vh' }}>
                  <TextField label="Comentário" multiline rows={4} variant="outlined" fullWidth margin="normal" value={editedComment} onChange={(e) => setEditedComment(e.target.value)} sx={{marginLeft: '2vh'}} />
