...
function App(props: AppProps) {
  ...
  const handleSaveAnime = async (anime: Anime, comment: string, rating: number) => {
    ...
    const response = await fetch(`${url}/favoritos/users/animes/${anime.mal_id}/save`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${localStorage.getItem('token')}`
  ...
  const handleOpenAddAnimeModal = async (anime: Anime) => {
      ...
      const response = await fetch(`${url}/favoritos/users/animes/${anime.mal_id}/check`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('token')}`
      ...
      <main>
        {isLoading ? ( // Affichage de l'indicateur de chargement
          ...
          <div className="welcome-message" style={{marginTop: "100px"}}>
            <h2>Bem-vindo aos seus favoritos!</h2>
            <p>Encontre Animes para adicionar a sua lista de favoritos na barra de pesquisa acima.</p>
          </div>
        ) : (
